...
(function () {
    ...
    async function loadTakenReminders() {
        ...
        const { data: reminders, error } = await db
            ...
            .eq('user_id', user.id).eq('status', 'taken')
        ...
        if (!reminders?.length) {
            reminderList.innerHTML = `<div style="text-align:center;padding:60px 20px;color:var(--gray);">
                <i class="fa-solid fa-circle-check" style="font-size:48px;opacity:.3;margin-bottom:16px;display:block;"></i>
                <p style="font-size:16px;font-weight:500;">No taken reminders yet</p></div>`;
            return;
        }
        reminders.forEach(r => reminderList.appendChild(buildTakenCard(r)));
    }
